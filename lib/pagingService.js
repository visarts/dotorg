...
const pagingService = () => {
  const literatureItemsFile = 'data/prod/literature-items.json';

  fs.readJson(literatureItemsFile, (error, result) => {
    const literatureItems = result;

    for (const index in literatureItems) {
      const item = literatureItems[index];
    ...
    fs.writeJson(literatureItemsFile, literatureItems);
